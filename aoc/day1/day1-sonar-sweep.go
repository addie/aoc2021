...
if you can help. Apparently, one of the Elves tripped and accidentally sent the sleigh keys flying into the ocean!

Before you know it, you're inside a submarine the Elves keep ready for situations like this. It's covered
in Christmas lights (because of course it is), and it even has an experimental antenna that should be able to
...
Your instincts tell you that in order to save Christmas, you'll need to get all fifty stars by December
25th.
...
func Solve(submit1, submit2 bool) error {
	filename := fmt.Sprintf(data.Filename, Day, Year)
	myData, err := data.Get(1, 2021, filename)
	if err != nil {
		return err
	}
	...
	level := 1
	res := solve(meta, level)
	if submit1 && res != 0 {
		err = data.Post(meta.day, meta.year, level, res)
		if err != nil {
			return err
		}
	}
